  ...
  return (
    ...
    <div className=" flex w-full justify-between items-center px-6 static z-20 lg:fixed bg-white ">
      ...
      <div className="hidden md:flex flex-nowrap">
        <div>
                    <input className=" border focus:outline-blue-200 border-gray-400 rounded-l-full p-4 h-10 lg:w-[30rem] [@media(max-width:768px)]:w-[10rem] md:w-[15rem]" 
            type="text"
            value={searchQuery}
          ...
          />
                    <button className="border border-gray-400 rounded-r-full w-16 h-10  bg-gray-200 text-center cursor-pointer">Search</button>
          
        </div>

        {showSuggestion && (
          <div className="fixed bg-white border border-gray-200 py-2 px-5 w-96 shadow-lg rounded-lg">
            <ul>
                            {suggestionResult && suggestionResult.map((suggestion, index) => (
                                <li className="shadow-sm py-2"
      ...
      <div className="flex justify-between items-center flex-nowrap">
        ...
        <text className="hidden md:inline-block w-24 p-2 rounded-full h-10 m-2 cursor-pointer">Sign in</text>
      </div>
            <div></div>
    </div>
    )
